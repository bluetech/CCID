...
again:
	...
	cmd[0] = CCID_MESSAGE_TYPE_PC_to_RDR_IccPowerOn;
	...
	unsigned char RxBuffer[], unsigned int *RxLength)
{
	...
	cmd[0] = CCID_MESSAGE_TYPE_PC_to_RDR_Secure;
	cmd[5] = ccid_descriptor->bCurrentSlotIndex;	/* slot number */
...
again:
	...
	bSeq = (*ccid_descriptor->pbSeq)++;
	cmd_in[0] = CCID_MESSAGE_TYPE_PC_to_RDR_Escape;
...
RESPONSECODE CmdPowerOff(unsigned int reader_index)
{
	...
	bSeq = (*ccid_descriptor->pbSeq)++;
	cmd[0] = CCID_MESSAGE_TYPE_PC_to_RDR_IccPowerOff;
	cmd[1] = cmd[2] = cmd[3] = cmd[4] = 0;	/* dwLength */
	cmd[5] = ccid_descriptor->bCurrentSlotIndex;	/* slot number */
	cmd[6] = bSeq;
...
again_status:
	...
	bSeq = (*ccid_descriptor->pbSeq)++;
	cmd[0] = CCID_MESSAGE_TYPE_PC_to_RDR_GetSlotStatus;
	cmd[1] = cmd[2] = cmd[3] = cmd[4] = 0;	/* dwLength */
	cmd[5] = ccid_descriptor->bCurrentSlotIndex;	/* slot number */
	cmd[6] = bSeq;
	cmd[7] = cmd[8] = cmd[9] = 0; /* RFU */
	...
	const unsigned char tx_buffer[], unsigned short rx_length, unsigned char bBWI)
{
	...
	cmd[0] = CCID_MESSAGE_TYPE_PC_to_RDR_XfrBlock;
	...
	unsigned int length, unsigned char buffer[])
{
	...
	bSeq = (*ccid_descriptor->pbSeq)++;
	cmd[0] = CCID_MESSAGE_TYPE_PC_to_RDR_SetParameters;
